...
public class SingleplayerManager : MonoBehaviour, IGameManager
{
    ...
    private void ShowNames()
    {
        ...
        {
            //Debug.Log(i == 0 ? PlayerPrefs.GetString("playerName") : $" омпьютер {i}");
            Scoreboard.Instance.SetPlayer(i, i == 0 ? PlayerPrefs.GetString("playerName") : $" омпьютер {i}");
        }
    }
    ...
    public int[] GetDice(int index)
    {
        ...
        bag.RemoveAt(index);
        // изменить переменную, ответственную за количество оставшихс€ фишек
    ...
    {
        // кол-во кост€шек, вкл code в теории - кол-во кост€шек, вкл code на доске и на руке у текущего компа
        // разделить
        // 71 - (кол-во кост€шек на доске + 4)
        //int SuitableDicesInTheory = 0;
        //int SuitableDicesInPractice = 0;
        //int DicesInTheory = 0;
        //int DicesInPractice = 0;
    ...
    private void CalculateMovesForDiceAndPlace(int diceIndex, int placeIndex, int[] code)
    {
        ...
        for (int i = 0; i < board.childCount; i++)
        {
            ...
            if (!place.CompareTag("Place"))
                continue;
            // не учитывать те, что р€дом с placeIndex
